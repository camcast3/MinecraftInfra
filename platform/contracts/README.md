...
```powershell
pwsh .\tools\validation\Validate-GameContracts.ps1
pwsh .\scripts\tests\Test-GameContracts.ps1
```

`contracts/` remains a byte-identical compatibility copy during the layout
migration. Validate it with `tools/layout/Sync-CompatibilityPaths.ps1`.
