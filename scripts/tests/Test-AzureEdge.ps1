...
$composePath = Join-Path $repositoryRoot 'platform/azure/edge/docker-compose.yml'
$networkBicepPath = Join-Path $repositoryRoot 'platform/azure/iac/modules/network.bicep'
$mainBicepPath = Join-Path $repositoryRoot 'platform/azure/iac/main.bicep'
$passed = 0
...
foreach ($contractName in @('palworld.json', 'windrose.json')) {
    $contractPath = Join-Path $repositoryRoot "platform/contracts/games/$contractName"
...
$firewallPath = Join-Path $repositoryRoot 'platform/azure/edge/reconcile-firewall.sh'
...
foreach ($backendBootstrap in @(
    'platform/proxmox/game-node/generated/palworld-cloud-init.yaml',
    'platform/proxmox/game-node/generated/windrose-cloud-init.yaml'
)) {
...
$vmBicepText = Get-Content `
    -LiteralPath (Join-Path $repositoryRoot 'platform/azure/iac/modules/vm.bicep') `
    -Raw -Encoding UTF8
Assert-True `
...
$refreshText = Get-Content `
    -LiteralPath (Join-Path $repositoryRoot 'platform/azure/edge/refresh-env.sh') `
    -Raw -Encoding UTF8
Assert-True `
