...
cat >"$test_root/mockbin/pgrep" <<'EOF'
#!/usr/bin/env bash
exit 1
EOF
...
PALWORLD_SHUTDOWN_WAIT_SECONDS=1
WINDROSE_STOP_TIMEOUT_SECONDS=1
...
EOF
  ...
  grep -Fq "destination=local status=success" "$backup_root/health/events.log"
  grep -Fq "destination=nas status=success" "$backup_root/health/events.log"
  grep -Fq "destination=azure status=success" "$backup_root/health/events.log"
  ...
  ! find "$backup_root" -name '*.partial.*' -print -quit | grep -q .

  printf '%s\n' unless-stopped >"$backup_root/health/${game}.restart-required"
  printf '%s\n' false >"$state/running"
  ...
  grep -Fq "destination=restart status=failure" "$backup_root/health/events.log"
  ...
  [[ ! -e "$backup_root/health/${game}.restart-required" ]]
  grep -Fq "destination=restart status=success" "$backup_root/health/events.log"
}

run_game palworld
run_game windrose

echo "Game backup lifecycle, retention, metadata, checksum, and restart failure tests passed."
