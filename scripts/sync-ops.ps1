...
param(
    [string]$WhitelistPath = [System.IO.Path]::Combine(
        $PSScriptRoot, '..', 'docker', 'shared', 'whitelist.json'
    ),
    [string]$OpsPath = [System.IO.Path]::Combine(
        $PSScriptRoot, '..', 'docker', 'shared', 'ops.json'
    ),
    [switch]$Check
)
...
if ($Check) {
    if (-not (Test-Path -LiteralPath $OpsPath -PathType Leaf)) {
        throw "Generated ops file not found: $OpsPath. Run scripts/sync-ops.ps1."
    }

    $actual = (Get-Content -Raw -LiteralPath $OpsPath -Encoding UTF8) -replace "`r`n", "`n"
    if ($actual -cne $expected) {
        throw "ops.json differs from whitelist.json. Run scripts/sync-ops.ps1 and commit the result."
    }
