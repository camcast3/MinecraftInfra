...
try {
    ...
    & (Join-Path $repositoryRoot 'tools/validation/Validate-GameContracts.ps1')
    & (Join-Path $repositoryRoot 'tools/layout/Test-RepositoryLayout.ps1')
    & (Join-Path $repositoryRoot 'scripts/tests/Test-GameContracts.ps1')
    & (Join-Path $repositoryRoot 'scripts/tests/Test-AzureEdge.ps1')
    & (Join-Path $repositoryRoot 'scripts/tests/Test-ReviewSafety.ps1')
    ...
    Invoke-Native -Command 'az' -Arguments @(
        'bicep', 'build',
        '--file', (Join-Path $repositoryRoot 'platform/azure/iac/main.bicep'),
        '--outfile', $bicepOutput,
        '--only-show-errors'
    )
    ...
    if ($AzureWhatIf) {
        ...
        Invoke-Native -Command 'az' -Arguments @(
            ...
            '--parameters', (Join-Path $repositoryRoot 'platform/azure/iac/parameters/prod.bicepparam'),
