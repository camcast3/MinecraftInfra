...
$TS_AUTH_KEY = Read-Host "  TailScale auth key (SINGLE-USE + PRE-APPROVED + NON-EPHEMERAL, from tailscale.com/admin/settings/keys — sidecar burns it on first registration; state at /data/minecraft/tailscale persists across restarts, so the key is intentionally one-shot for security)"
Invoke-Az @('keyvault', 'secret', 'set',
...
Write-Host "      TS_AUTHKEY                 — REUSABLE key from tailscale.com/admin/settings/keys"
Write-Host "                                   (Proxmox stack re-injects via Portainer env on"
Write-Host "                                    each redeploy, so reusable is appropriate;"
Write-Host "                                    the Azure VM uses a SINGLE-USE key on a"
Write-Host "                                    different code path — see Step 7.)"
