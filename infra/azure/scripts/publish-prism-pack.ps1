...
if (-not $SkipDriftCheck -and -not $LocalMode) {
    ...
    $syncOpsScript = Join-Path $repoRoot 'tools/access/sync-ops.ps1'
    & $syncOpsScript -Check
    & (Join-Path $repoRoot 'tools/layout/Test-RepositoryLayout.ps1')
}
...
"@
        ...
        $composeFile = Join-Path $repoRoot 'games/minecraft/c2e2/docker-compose.yml'
        ...
        Write-Ok "Rewrote games/minecraft/c2e2/docker-compose.yml: PACKWIZ_URL pinned to $packwizSha, MOTD pinned to v$Version"
        ...
        $velocityTmpl = Join-Path $repoRoot 'platform/azure/edge/velocity/velocity.toml.tmpl'
        ...
        Write-Ok "Rewrote platform/azure/edge/velocity/velocity.toml.tmpl: fallback motd pinned to v$Version"

        & (Join-Path $repoRoot 'tools/layout/Sync-CompatibilityPaths.ps1') -Write
        git add modpack.yml `
            'games/minecraft/c2e2/docker-compose.yml' `
            'docker/proxmox/docker-compose.yml' `
            'platform/azure/edge/velocity/velocity.toml.tmpl' `
