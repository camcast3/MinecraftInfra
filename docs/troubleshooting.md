...
| Custom mods or config tweaks reverted after update | The update step replaces anything not in the pack-author preserve list. This is **expected** — the server kicks players with mismatched mods anyway. If you want to test custom mods locally, uncheck **Custom commands** in the instance's **Edit → Settings → Custom commands** tab (you won't be able to connect to the live server while it's unchecked). |
