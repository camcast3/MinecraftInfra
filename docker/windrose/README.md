...
`games/windrose/docker-compose.yml` owns the stack. Portainer still consumes the
byte-identical `docker/windrose/docker-compose.yml` compatibility path. Set these values in Portainer's stack
environment UI:
...
on 7777. The Tailscale sidecar has an independent health check. The Azure edge
forwards both protocols to the Key Vault-configured stack tailnet IPv4; do not
open a home-router port or add 7778 unless official documentation later
requires it.
