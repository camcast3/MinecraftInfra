...
if ($SyntheticFixture) {
    $fixtureRoot = [IO.Path]::GetFullPath((Join-Path $repoRoot '.artifacts')).TrimEnd(
        [IO.Path]::DirectorySeparatorChar) + [IO.Path]::DirectorySeparatorChar
    foreach ($root in $InstancesRoot) {
        if (-not $root.StartsWith($fixtureRoot, [StringComparison]::OrdinalIgnoreCase)) {
            throw '-SyntheticFixture can only read instance roots under the repository .artifacts directory.'
        }
    }
}
...
foreach ($candidate in $candidates) {
    ...
    if (-not $SyntheticFixture -and $activeProcesses.Count -gt 0) {
        $case.status = 'skipped'
